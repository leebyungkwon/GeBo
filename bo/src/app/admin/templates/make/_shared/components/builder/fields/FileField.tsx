...
/**
 * FileField: 파일 업로드 설정용 L3 컴포넌트
 */
export const FileField = (props: FieldEditProps) => {
        ...
        readonly={values.readonly}>
            <div className="space-y-3 pt-1 border-t border-slate-100 mt-1">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            onChange={e => {
                                ...
                                if ((values.maxTotalSizeMB ?? 20) < v) updates.maxTotalSizeMB = v;
                                onChange(updates);
                            }}
                            className="w-full border border-slate-200 rounded px-2 py-1.5 text-xs focus:outline-none focus:border-slate-900"
                        />
                ...
                <div>
                    ...
                    <input
                        type="number" min={1}
                        value={values.maxTotalSizeMB ?? 20}
                        onChange={e => {
                            const v = Math.max(Number(e.target.value) || 1, values.maxFileSizeMB ?? 10);
                            onChange({ maxTotalSizeMB: v });
                        }}
                        className="w-full border border-slate-200 rounded px-2 py-1.5 text-xs focus:outline-none focus:border-slate-900"
                    />
                </div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        {(['', 'doc', 'image', 'custom'] as const).map(mode => (
                            <label key={mode} className="flex items-center gap-1.5 cursor-pointer group">
                                ...
                                />
                                <span className={`text-[11px] font-medium transition-colors ${(values.fileTypeMode ?? '') === mode ? 'text-slate-900' : 'text-slate-500 group-hover:text-slate-700'}`}>
